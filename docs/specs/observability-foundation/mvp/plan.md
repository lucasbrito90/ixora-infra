...
**Technical debt documented (post-completion, 2026-08-22):** TD-5 — HTTP recording rules (`ixora:sli:http:*`, `ixora:slo:http*`, `ixora:slo:http_latency*`) structurally cannot populate under the current metrics pipeline; live traffic validation against both local dev and real staging confirmed every `ixora_http_server_request_total` sample reports `1` regardless of volume, so `rate()`/`increase()` always evaluate `0` and the dependent SLI/SLO ratios stay `NaN`. Root cause: FrankenPHP runs with worker mode off (`back_vibes/docker/frankenphp/Caddyfile`), so the OTel PHP SDK's meter state resets every request, and the collector has no `cumulativetodelta`/`deltatocumulative` processor to reconstruct a real cumulative counter server-side (`ixora-infra/collector/config.yaml`). Severity: Medium — D-08's 3 HTTP SLIs (availability, latency) and D-05's raw HTTP panels are affected; the 4 HTTP-related SLO burn-rate alerts will never fire since their inputs are always `NaN`, not because traffic is healthy. Queue/Scheduler/Smart Home SLIs are unaffected (already persistent-process workers). Two candidate fixes identified (FrankenPHP/Octane worker mode vs. collector-side delta accumulation processors). **Update (2026-08-22):** the collector-side fix was prototyped and validated locally (collector + Prometheus, no Grafana available locally) and **found not to work** — `cumulativetodelta`'s reset detection is value-based only (doesn't use `StartTimestamp`), and since `back_vibes` always resets to the same value (`1`) it never sees a detectable drop, so deltas compute as `0`. The PHP SDK also ignores `OTEL_EXPORTER_OTLP_METRICS_TEMPORALITY_PREFERENCE=delta`, so a config-only route to delta-native export doesn't exist either. The config change was reverted (not merged). **FrankenPHP/Octane worker mode is now the only validated fix** for TD-5. **Final update (2026-08-22): shipped and validated live in staging** (`back_vibes#2` + `tofu apply`) — `ixora:sli:http:ratio:rate5m` and `ixora:slo:http:burn_rate:5m` are now numeric (not `NaN`) against real staging traffic. Along the way, discovered and fixed an unrelated incident: the `tofu apply` had wiped out-of-band `OTEL_*` env vars that were never captured in Terraform (dashboard-only config); restored them into `terraform.tfvars` so they're IaC-managed going forward. TD-5 is closed. Full detail: `tasks.md` Phase 8.9 notes (P8.9-14 through P8.9-16).

**Branch:** `feature/observability-slo-error-budget`

---

**TD-6 (new, High severity, discovered 2026-08-23 while live-validating Phase 7B.5's push metric in staging):** queue worker metrics and traces never flush to the collector — only queue-worker logs actually reach the backend. Root cause: `Illuminate\Queue\Worker::kill()` self-`SIGKILL`s on every graceful shutdown (every redeploy, every `queue:restart`), bypassing `app()->terminate()` and therefore the only `TelemetryManager::flush()` call site in the codebase. Platform-wide impact, not push-specific: `ixora_queue_job_total`/`.duration` (D-04), `ixora_smart_home_dispatch_total`, `ixora_smart_home_action_total`/`.duration` (D-02), and `ixora_push_delivery_total` (D-03) have likely never populated from real queue traffic, and the non-HTTP D-08 SLO burn-rate alerts (`ixora:slo:queue:*`, `ixora:slo:smart_home:*`, `ixora:slo:scheduler:*`) have had no real data to evaluate since Phase 7B.2/7B.4 shipped. Fix (flush after every job via `JobProcessed`/`JobFailed`/`JobExceptionOccurred`/`JobTimedOut`, plus `WorkerStopping` as a safety net) implemented, validated in a faithful local reproduction, then shipped and validated live in staging (2026-08-23): `ixora_push_delivery_total` and `ixora_queue_job_total` both confirmed populating in real staging Prometheus after a fresh dispatched push notification. **TD-6 is closed.** Full detail: `tasks.md` Phase 8.9 notes.
...
#### Phase 11 — Production Readiness Review — **Review complete; decision NO-GO; remediation partially closed**
...
**Headline finding — the actual blocker:** `ixora-infra/opentofu/` contains only a `staging/` environment. **No production infrastructure is defined as code at all.** Every other finding below is a fixable gap on infrastructure that already exists; this one means there is currently no environment to evaluate production readiness against. **Explicitly deferred by operator decision** ("Pode seguir com tudo menos produção real, por enquanto iremos ter somente o ambiente de staging. Irei me preocupar quando tiver um MVP do produto") — remains the sole open item blocking a Go decision.

**Other findings — remediation status:**

- **Backups: architecture-only.** ~~`backup-strategy.md`'s own header states no backups are implemented — no automation, no tested restore, for either the observability stack or (documented, at least) the managed Postgres cluster's backup behavior.~~ **RESOLVED (2026-08-30):** `scripts/backup-observability-volumes.sh` / `scripts/restore-observability-volume.sh` deliver the staging tarball backup + restore automation the design called for, covering all 4 observability volumes with 4-generation retention; full backup→corrupt→restore→health-check cycle verified locally. Merged via PR #25 → `develop` → `staging`. Managed Postgres itself already had automatic daily backups confirmed via `doctl` earlier in this review — that part of the finding was downgraded to a documentation gap, not a real gap, before this remediation.
- **No rate limiting anywhere in `back_vibes`'s API** — confirmed by grepping every middleware/provider for `throttle`/`RateLimiter::for`. The Firebase auth endpoints (`/api/auth/firebase`, `/api/auth/sync`) have no request-rate protection. **RESOLVED (2026-08-30):** `AppServiceProvider::configureRateLimiting()` adds a 10/min-per-IP `auth` limiter on both auth endpoints and a 60/min-per-user `api` limiter on the rest of the authenticated API, covered by 6 new Pest tests (full suite 1043/1043 passing). Merged via PR #7 → `develop` → `staging`.
- **Real, previously-unnoticed finding:** `back_vibes/.github/workflows/deploy-staging.yml`, a legacy SSH-based deploy path superseded by App Platform, is still active, still triggers on every push to `staging`, and has failed on its 5 most recent runs (2026-08-03 through 2026-08-25) — almost certainly at the SSH connection step, since its target Droplet from the pre-App-Platform era likely no longer exists. `deploy-pipeline.md` had already flagged this exact dual-deploy-path risk as tech debt; it just hadn't been acted on. Dead, consistently-red CI, unaddressed for 3+ weeks. **RESOLVED (2026-08-30):** workflow removed and its now-orphaned setup guide (`docs/deploy-staging.md`) replaced with a note pointing at the real App Platform pipeline, in the same PR (#7) as rate limiting.
...
**Decision:** **NO-GO** (as originally documented in `production-readiness-review.md`). Of the prioritized "what Go requires" list, backups, rate limiting, and the dead CI workflow are now closed; the production OpenTofu environment remains the sole outstanding item, deferred until the product reaches an MVP milestone per operator decision.

**Result:** The review itself (§P11-1/P11-2 in `tasks.md`) is complete and the decision is documented. Of the three fixable-now findings under §P11-3, all three are closed and deployed to real staging (2026-08-30). The remaining blocker — no production infrastructure — is not tracked as pending work; it is intentionally out of scope until an MVP milestone, per explicit operator instruction.
...
#### Phase 8.8.6 — Observability Infrastructure Hardening — **Complete**

**Goal:** Production-grade operational hardening without redesigning the Phase 8.8.5 architecture.

**Deliverables:**

- **`observability-hardening.md`:** Hardening index + architecture review.
- **`deployment-strategy.md`:** Immutable release-tag deployments (replaces `git pull develop` as recommended workflow).
- **`backup-strategy.md`:** Backup architecture for Prometheus/Loki/Tempo/Grafana (design only).
- **`storage-strategy.md`:** Strategy A retained; Strategy B + Reserved IP documented.
- **`future-cicd.md`:** Future GitHub Actions → SSH → deploy pipeline architecture.
- **`cloud-init-review.md`:** cloud-init limitations + OpenTofu lifecycle review.
- **`app-platform-otel-integration.md`:** Future App Platform OTEL automation design.
- **Optional Reserved IP:** `observability_use_reserved_ip` (default `false`) in OpenTofu.
- **`deploy-observability.sh`:** `IXORA_GIT_REF` support for release deployments.
- **validate.sh extended to 90/90 checks** (checks 79–90).

**Result:** Documentation-only hardening complete. No backups, CI/CD, or App Platform changes implemented.
...
**Known limitation — RESOLVED (2026-08-30):** the apparent `IonTabs` bottom-tab navigation gap was investigated by patching `history.pushState`/`replaceState` via Chrome DevTools Protocol (no app code changes) to observe navigation independent of OTel entirely. Root cause: `adb shell input tap` did not reliably register as a click on `ion-tab-button` — a plain content-area button responded correctly to the identical synthetic tap. A real human finger tap on the device confirmed both navigation and telemetry work correctly for every tab switch (4 real `screen.*` trace exports captured, independently verified via the Grafana MCP). The same investigation also produced a real distributed trace spanning `front_vibes` (client fetch span) → `back_vibes` (Laravel server span, parent/child-linked) → Eloquent → raw PostgreSQL queries → Firebase JWT-verification calls to Google — all under one `traceID`, requiring zero code changes (existing `propagateTraceHeaderCorsUrls` config + back_vibes's existing OTel PHP SDK were already sufficient). Evidence: `qa/observability-mobile-sdk/evidence-iontabs-investigation-and-distributed-trace-2026-08-30.txt`. Trello card closed as "not a bug."
