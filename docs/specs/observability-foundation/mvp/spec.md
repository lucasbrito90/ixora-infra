...
| **7B.2** | Queue + Console (`back_vibes`) — manual spans/metrics for queue jobs and console commands |
| **7B.3** | Scheduler (`back_vibes`) — manual spans/metrics for Scheduler dispatch |
| **7B.4** | Smart Home (`back_vibes`) — manual spans/metrics for Smart Home provider adapter calls |
| **7B.5** | Push Notifications (`back_vibes`) — manual spans/metrics for push delivery |
| **7B.6** | External Providers (`back_vibes`) — manual spans/metrics for outbound provider integrations |
