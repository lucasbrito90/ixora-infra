...
| KL-IR-6 | **Confirmed root cause (2026-08-30): a new alert joining an already-active notification group waits for the group's next scheduled `group_interval` flush, not `group_wait`.** `group_wait` (2m for the `warning` route) only applies to a group's very first-ever notification. Once a group exists, Alertmanager's dispatcher flushes it on a fixed `group_interval` cadence (15m for `warning`) regardless of when new members join. Reproduced with `GF_LOG_LEVEL=debug` on the real host: the `{severity="warning"}:{category="application", environment="staging"}` group last flushed at `00:04:25`; a genuinely firing alert joined it at `00:10:40`; the group's next scheduled flush wasn't until `00:19:25` (exactly 15m after the prior flush) — an **8m45s** wait, confirmed by the Mailtrap-delivered email's `sent_at` matching `00:19:25` to the second. The group stays "active" because the recurring, silenced `DatasourceNoData` alerts for `category=application` share the same group labels and keep re-flushing it every cycle | For `warning` this can add up to ~15 minutes of delay before a real alert reaches an inbox. For `critical`/`emergency` (`group_interval: 5m` on that route), the same mechanism could add up to ~5 minutes — meaningful against the §3 response-time targets (15–30 min for critical, immediate for emergency), though less severe than for warning. **Fixed in `feature/observability-notification-grouping-fix`:** a dedicated `alertname = DatasourceNoData` route was added as the *first* route in `collector/grafana/provisioning/alerting/notification-policies.yaml`, with `group_by: [alertname, rulename]` and `continue: false`, so every NoData alert forms its own per-rule group and never reaches the severity-based routes below. Validated locally: `validate.sh` 105/105 PASS (check 105 is a regression guard for this ordering); Grafana provisioning API confirms the route is index 0. **Deployed to the real staging host and timing-verified 2026-08-30**: after a `docker compose up -d --force-recreate grafana` (required — the provisioning file is bind-mounted, so a plain `up -d` does not reload it) and a fresh `GF_LOG_LEVEL=debug` repro, the dispatcher log showed the alert `Received` at `02:07:40` and the group `flushing` at `02:09:40` — exactly the `group_wait: 2m` for the warning route, not the 15m seen before the fix. This is the authoritative confirmation (Alertmanager's own dispatcher decision), independent of final SMTP delivery. **Caveat found while verifying:** the Mailtrap sandbox inbox's own `last_message_sent_at` counter stopped advancing partway through this investigation (~44 messages sent same-day) and the corresponding email was never captured, even though the dispatcher log confirms Grafana attempted to send it — consistent with the Mailtrap sandbox's own sending limits being hit from the volume of testing done today, not a defect in this fix. A real SMTP relay (KL-IR-3) would not have this ceiling. |
