...
| **Observability Foundation** | Phase 1 + 1.5 + 2 + 2.5 + 9.5 + 3 + 3.5 + 3.75 + 4 + 5 + 5.5 + 6 + 6.5 + **7A** + **7B.1** + **7B.2** + **7B.3** + **7B.4.1** + **7B.4.2** + **7B.4.3** + **7B.4.4** + **7B.4.5** + **7B.4.6** + **7B.4.7** + **7B.4.8** + **7B.4.9** + **8.0** + **8.1** — Backend SDK Foundation, HTTP + Routing, Queue + Console, and generic Scheduler telemetry shipped in `back_vibes`; Smart Home Business Telemetry complete and validated; Business Telemetry Foundation Baseline established as platform-wide standard; Dashboard Requirements Review complete — 7 dashboards defined, 6 investigation workflows documented, Phase 9 implementation checklist ready; **Grafana Foundation & Provisioning** complete — Grafana OSS active, 3 datasources with stable UIDs provisioned, 4 folder providers defined, 12/12 validation checks pass; ready for Phase 8.2 ([grafana-foundation](specs/observability-foundation/mvp/grafana-foundation.md); [dashboard-requirements](specs/observability-foundation/mvp/dashboard-requirements.md); [business-telemetry-foundation](specs/observability-foundation/business-telemetry/business-telemetry-foundation.md); ADRs 028–031) |
...
*Last indexed: documentation tree under `ixora-infra/docs/`. Last updated: 2026-07-19 (observability-foundation Phase 8.1 — Grafana Foundation & Provisioning).*
