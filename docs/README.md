...
| **Observability Foundation** | Phase 1 + 1.5 + 2 + 2.5 + 9.5 + 3 + 3.5 + 3.75 + 4 + 5 + 5.5 + 6 + 6.5 + **7A** + **7B.1** + **7B.2** + **7B.3** + **7B.4.1** + **7B.4.2** + **7B.4.3** + **7B.4.4** + **7B.4.5** + **7B.4.6** + **7B.4.7** — Backend SDK Foundation, HTTP + Routing, Queue + Console, and generic Scheduler telemetry shipped in `back_vibes`; Business Telemetry domain execution review complete; Smart Home dispatch boundary (`smart_home.dispatch` span), Action Execution boundary (`smart_home.action` span), Provider Boundary (`smart_home.provider` span), Business Failure Semantics (formal failure taxonomy + one span-status correction), Business Metrics (`ixora.smart_home.dispatch.total`, `ixora.smart_home.action.total`/`.duration`), and Business Logging (L-2 resolution + log sanitization, `back_vibes`) shipped; ready for Phase 7B.5 ([backend-sdk-foundation](specs/observability-foundation/mvp/backend-sdk-foundation.md); ADRs 028–031) |
...
## Observability Foundation (Phase 1 + 1.5 + 2 + 2.5 + 9.5 + 3 + 3.5 + 3.75 + 4 + 5 + 5.5 + 6 + 6.5 + 7A + 7B.1 + 7B.2 + 7B.3 + 7B.4.1 + 7B.4.2 + 7B.4.3 + 7B.4.4 + 7B.4.5 + 7B.4.6 + 7B.4.7)
...
*Last indexed: documentation tree under `ixora-infra/docs/`. Last updated: 2026-07-18 (observability-foundation Phase 7B.4.7 — Business Logging).*
