...
| **Observability Foundation** | Phase 1 + 1.5 + 2 + 2.5 + 9.5 + 3 + 3.5 + 3.75 + 4 + 5 + 5.5 + 6 + 6.5 + **7A** + **7B.1** + **7B.2** + **7B.3** + **7B.4.1** + **7B.4.2** + **7B.4.3** — Backend SDK Foundation, HTTP + Routing, Queue + Console, and generic Scheduler telemetry shipped in `back_vibes`; Business Telemetry domain execution review complete; Smart Home dispatch boundary (`smart_home.dispatch` span) and Action Execution boundary (`smart_home.action` span, `back_vibes`) shipped; ready for Phase 7B.4.4 (Provider Boundary) ([backend-sdk-foundation](specs/observability-foundation/mvp/backend-sdk-foundation.md); ADRs 028–031) |
...
| 3 | **Observability Foundation** | **Phase 7A, 7B.1, 7B.2, 7B.3 complete — Backend SDK Foundation + HTTP/Routing + Queue/Console + generic Scheduler (`back_vibes`); Phase 7B.4.1 (Business Telemetry domain execution review) complete — discovery only; Phase 7B.4.2 (Smart Home dispatch boundary) complete — `smart_home.dispatch` span; Phase 7B.4.3 (Smart Home Action Execution) complete — `smart_home.action` span (`back_vibes`); Phase 7B.4.4 (Provider Boundary) next** | [backend-sdk-foundation](specs/observability-foundation/mvp/backend-sdk-foundation.md) · [backend-queue-console-instrumentation](specs/observability-foundation/mvp/backend-queue-console-instrumentation.md) · [backend-generic-scheduler-instrumentation](specs/observability-foundation/mvp/backend-generic-scheduler-instrumentation.md) · [domain-execution-review](specs/observability-foundation/business-telemetry/domain-execution-review.md) · [backend-smart-home-dispatch-boundary](specs/observability-foundation/business-telemetry/backend-smart-home-dispatch-boundary.md) · [backend-smart-home-action-execution](specs/observability-foundation/business-telemetry/backend-smart-home-action-execution.md) · [traces-philosophy](architecture/traces-philosophy.md) · [tempo-deployment](specs/observability-foundation/mvp/tempo-deployment.md) · [metrics-philosophy](architecture/metrics-philosophy.md) |
...
## Observability Foundation (Phase 1 + 1.5 + 2 + 2.5 + 9.5 + 3 + 3.5 + 3.75 + 4 + 5 + 5.5 + 6 + 6.5 + 7A + 7B.1 + 7B.2 + 7B.3 + 7B.4.1 + 7B.4.2 + 7B.4.3)
...
*Last indexed: documentation tree under `ixora-infra/docs/`. Last updated: 2026-07-17 (observability-foundation Phase 7B.4.3 — Smart Home Action Execution).*
