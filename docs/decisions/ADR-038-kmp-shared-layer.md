...
**Post-acceptance addendum (2026-09-25).** Phase 1 (K07–K11) of the migration completed and produced three factual updates that affect the *justifications* behind Decisions 4 and 5 — not the decisions themselves. No decision is reopened. Status remains Accepted.

1. **Decision 5, iOS compilation justification corrected.** The original wording states "there is no Mac, so `commonMain` cannot be verified by compiling for iOS. The test is the only mechanism…". Phase 1 showed that `compileKotlinIosArm64` and `compileTestKotlinIosArm64` **do execute** on Windows with Kotlin 2.3.20: the compiler cross-compiles K/Native targets without a Mac; only the iOS linker step and device tests are skipped. A deliberate probe in an isolated, discarded clone outside the repository confirmed that `java.io.File`, `java.time.LocalDate.now()` (FQN, no import) and `android.util.Log` each fail `compileKotlinIosArm64` with *Unresolved reference*. The iOS compilation layer therefore provides real enforcement. Corrected justification: **the boundary test is not the only enforcement mechanism; it is the layer that covers what the compiler does not cover.** Two reasons the scanner remains necessary at full strength follow from the phase: (a) the compiler rejects JVM/Android FQNs but does not prevent an `androidx.*` library from being added as a Gradle dependency when it publishes iOS-variant klibs — and when such a library is added with a source file importing `androidx.lifecycle.ViewModel`, `compileKotlinIosArm64` **succeeded** (the compiler accepted it), while `CommonMainBoundaryTest` **failed** reporting `androidx.*`. This was observed in a discarded clone during K12 (2026-09-25). The scanner is therefore the only mechanism that rejects this case. (b) the scanner is independent of Kotlin toolchain behaviour and provides a stable, version-agnostic check that the compiler's rules alone cannot guarantee across upgrades. The original risk accepted in §9(5) is unchanged.

2. **`androidx.*` tension with the responsibility matrix.** Decision 2 (line ~95) lists "Non-sensitive preferences ● … DataStore is multiplatform". `androidx.datastore` is an `androidx.*` package, which Decision 5 prohibits in `commonMain`. The PO resolved this tension in K08 by applying Decision 5 literally — no allowlist, no special-case — accepting that the persistence technology for non-sensitive preferences will be chosen in ADR-043 (Phase 3), where the tension may dissolve if the final choice is not `androidx.datastore`. **This conflict is registered here and attributed to ADR-043 for resolution.** Nothing in this addendum changes either decision.

3. **Nomenclature note.** Decision 8 and the plan refer to the Android target as `androidTarget()` (the KMP Gradle DSL name). The implementation uses the `com.android.kotlin.multiplatform.library` plugin (AGP 8.10+), which exposes the target as `android { }` inside the `kotlin { }` block rather than `androidTarget()`. This is a naming difference between the plugin API and the KMP documentation — not an architectural divergence. The decision is unaffected.
