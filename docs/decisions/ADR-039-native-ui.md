...
**Accepted** (2026-09-23; post-acceptance addendum 2026-09-25) — governs how the Ixora mobile UI is implemented on each platform, and how the existing design language is preserved across that boundary.
...
**Post-acceptance addendum (2026-09-25).** The PO decided that the mobile application receives a **new visual identity** — modern and relaxing, inspired by the mood, shapes and typography of a licensed reference kit (Sleepie), with Ixora's own identity, colours, fonts and voice; no screens, illustrations or assets of the kit are reproduced. The result is the **Ixora Design System v1**, approved by the PO on 2026-09-25 (tokens, brand book, components, icons, motion guide, and key screens: Home, Player, My Vibes, Settings and authentication). This addendum records what that decision changes in this ADR. Status remains Accepted.

1. **Decision 8 no longer applies to the visual layer.** The feature freeze continues to cover **behaviour and functionality** (plan D2): no new features and no new screens beyond those the existing app already has. The **visual experience is now a deliberate product change**, decided by the PO. The sentence "Migration ≠ Redesign" in §2.1 and the parity rule in Decision 8 therefore stop holding for visuals: "the existing token values are parity" and "the existing component set is parity" are superseded. Theming `system`/`light`/`dark` remains as behaviour parity.
2. **Decision 4 is superseded for visual values.** The source of the tokens is no longer `variables.css` and `motion.css` of `front_vibes`, but the `tokens.json` of the Ixora Design System v1 (colour in light and dark, typography, spacing, radius, shadow) and its motion guide, delivered by the design task. The consolidation rule (exact literal → token; unmatched literal → preserved and recorded, never rounded) no longer applies to the port; its underlying discipline **does** carry over: the design tokens are ported **exactly** into `Colors.kt`, `Typography.kt`, `Dimensions.kt` and `IxoraTheme.kt` (and, later, their Swift equivalents), without rounding or reinterpretation. "No automatic Figma → code synchronization" is unchanged; the origin of the visual language is now the Ixora Design System v1, not Figma node 127:2. The reference kit is style inspiration only.
3. **Decisions 1, 2, 3, 5, 6 and 7 are unchanged.** Native UI on each platform, no Compose Multiplatform, same design language with native implementations, the platform structure, accessibility as a Design System responsibility, the prohibited abstractions, and Android first all stand. iOS remains SwiftUI, prepared and not built. The Design System v1 is written for both: it names the Compose and SwiftUI equivalents where relevant.
4. **What the Design System adds that this ADR did not decide.** Two fonts (Outfit and DM Sans, open-licensed) must be bundled as font resources in each platform's app; the icon family is Lucide (ISC/MIT), delivered as vector assets. Accessibility criteria are stated per component with WCAG AA contrast ratios measured for every token pair in light and dark; native verification with TalkBack and VoiceOver is still to be done.
5. **Coverage.** Screens not yet drawn in Design System v1 (Devices, Smart Home connection, Presets, create and edit vibe, sound lists, schedules, scenes, player menu) are designed as their UI area is reached in Phase 6, using only Design System components and tokens; anything that needs a new component returns as a change to the Design System.
