...
**Status:** Concluída (2026-08-30) — cenários de leitura e escrita entregues e testados de verdade contra staging, nenhum gargalo encontrado em nenhum dos dois, e a conclusão "sem gargalo nos níveis conservadores testados" foi formalmente aceita pelo operador como fechamento da fase.  
**Progresso:** **7/7 - 100%**

Descrição: validar desempenho, estabilidade e capacidade do Ixora sob carga realista antes da preparação para produção. Entregue: scripts k6 (`ixora-infra/qa/load-testing/`) cobrindo leitura (/api/health, /vibes, /schedules, /sounds, /preset-vibes) e escrita completa — CRUD de vibes e schedules com limpeza garantida por iteração (nada fica órfão, verificado de forma independente duas vezes via login Firebase real + GET). Reaproveita o fluxo de autenticação Firebase já usado em `qa/scheduler-e2e/`. Smoke e baseline rodados contra o staging real para os dois cenários: leitura (3 VUs/10 iters, 63/63 checks, p(95)=645ms) e escrita (2 VUs/10 iters, 73/73 checks, p(95)=524ms) — ambos sem nenhum impacto observado (nenhum alerta disparou, `/api/health` respondeu 200 depois). Achado real: o volume gerado não foi suficiente pra tirar as regras de alerta HTTP da Phase 9 do estado `NoData` (janelas de 5 minutos) — dado útil pra ajuste futuro de threshold. Achado de ambiente: o staging real roda em App Platform `basic-xxs` (menor tier) com Postgres `db-s-1vcpu-1gb` de nó único — não é um ambiente dimensionado para achar um teto real de capacidade. **Conclusão formal (`qa/load-testing/README.md` §"Formal conclusion"):** todos os 4 testes passaram com folga contra as metas dos scripts (p95<3000ms, erro<5%, checks>95% — p95 real nunca passou de 823ms); em vez de subir VUs pra achar o teto desse tier específico (que não seria representativo de produção e traria risco desnecessário ao staging compartilhado), o operador aceitou formalmente "sem gargalo nos níveis conservadores testados" como conclusão da fase. Fluxos de Smart Home/push, dashboard no Grafana pras métricas k6, e qualquer teste futuro de capacidade real ficam registrados como fora de escopo deste fechamento, não como gargalos em aberto.

## Phase 11 - Production Readiness Review

**Status:** Revisão concluída (30/08/2026) — decisão formal: **NO-GO** para produção; remediação **parcialmente concluída** (30/08/2026)  
**Progresso:** **Remediação parcial — 3 dos 4 achados críticos fechados; infra de produção adiada até MVP por decisão do operador**

Descrição: revisão formal da prontidão para produção, cobrindo `back_vibes` e `ixora-infra` (segurança, confiabilidade, backups, migrations, secrets, rollback e riscos abertos). Achado principal: `ixora-infra/opentofu/` só tem um ambiente `staging/` — **não existe nenhuma infraestrutura de produção definida como código**. Este item foi **explicitamente adiado por decisão do operador** ("Pode seguir com tudo menos produção real... irei me preocupar quando tiver um MVP do produto") e não é tratado como pendência aberta enquanto isso. Outros achados, todos **remediados em 30/08/2026**: backups eram só arquitetura, nunca implementados — agora `scripts/backup-observability-volumes.sh`/`restore-observability-volume.sh` automatizam backup+restore local dos 4 volumes de observabilidade, testado ponta a ponta (PR #25, `ixora-infra`); nenhum rate limiting em nenhum endpoint da API do `back_vibes` — agora há limiters `auth` (10/min por IP) e `api` (60/min por usuário) via `RateLimiter::for`, com 6 testes novos (PR #7, `back_vibes`); e o workflow antigo de deploy via SSH (`back_vibes/.github/workflows/deploy-staging.yml`), superado pelo App Platform e falhando em toda execução há semanas, foi removido junto com seu guia de setup órfão (mesmo PR #7). Gestão de secrets, migrations e rollback passaram bem na revisão (documentados e corretos). Decisão, checklist e remediação completos em `ixora-infra/docs/specs/observability-foundation/mvp/production-readiness-review.md` e `tasks.md`/`plan.md` (P11-3).
